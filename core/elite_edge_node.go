...
func init() {
	// Each elite edge node stake deposit needs to be at least 20,000 DToken
	MinEliteEdgeNodeStakeDeposit = new(big.Int).Mul(new(big.Int).SetUint64(20000), new(big.Int).SetUint64(1e18))

	// Each elite edge node stake deposit should not exceed 500,000 DToken
