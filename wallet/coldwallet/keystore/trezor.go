...
import (
	...
	tp "github.com/dnerochain/dnero/ledger/types"
	"github.com/dnerochain/dnero/wallet/coldwallet/keystore/trezor"
...
// or whether the Dnero app was not started on it.
...
// and returning the Dnero address located on that derivation path.
...
// Dnero address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	...
	request := &trezor.DneroGetAddress{
...
func (w *trezorDriver) trezorSignMsg(derivationPath []uint32, txrlp common.Bytes) (common.Address, *crypto.Signature, error) {
	...
	request := &trezor.DneroSignMessage{
		AddressN: derivationPath,
		Message:  txrlp,
	}
	...
	response := res.(*trezor.DneroMessageSignature)
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, txrlp common.Bytes) (common.Address, *crypto.Signature, error) {
	...
	response := res.(*trezor.DneroTxRequest)

	for response.DataLength != 0 && int(response.DataLength) <= len(data) {
		chunk := data[:response.DataLength]
		data = data[response.DataLength:]

		request := &trezor.DneroTxAck{DataChunk: chunk}
	...
	// Extract the Dnero signature and do a sanity validation
