...
func (t *DneroCliRPCService) Send(args *SendArgs, result *SendResult) (err error) {
	...
	dfuelwei, ok := new(big.Int).SetString(args.DFuelWei, 10)
	if !ok {
		return fmt.Errorf("Failed to parse dfuelwei: %v", args.DFuelWei)
	}
	...
	inputs := []types.TxInput{{
		Address: from,
		Coins: types.Coins{
			DFuelWei: new(big.Int).Add(dfuelwei, fee),
			DneroWei: dnerowei,
		},
		Sequence: sequence,
	}}
