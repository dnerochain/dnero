...
import (
	...
	wl "github.com/dnerochain/dnero/wallet"
	wt "github.com/dnerochain/dnero/wallet/types"
...
func NewDneroCliRPCServer(cfgPath, port string) (*DneroCliRPCServer, error) {
	...
	t := &DneroCliRPCServer{
		DneroCliRPCService: &DneroCliRPCService{
			wallet: wallet,
			wg:     &sync.WaitGroup{},
		},
		port: port,
	...
	s := rpc.NewServer()
	s.RegisterName("dnerocli", t.DneroCliRPCService)
...
func (t *DneroCliRPCServer) mainLoop() {
	defer t.wg.Done()
...
func (t *DneroCliRPCServer) serve() {
