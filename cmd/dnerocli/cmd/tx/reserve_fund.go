...
func doReserveFundCmd(cmd *cobra.Command, args []string) {
	...
	fund, ok := types.ParseCoinAmount(reserveFundInDFuelFlag)
	if !ok {
		utils.Error("Failed to parse fund")
	}
	col, ok := types.ParseCoinAmount(reserveCollateralInDFuelFlag)
	if !ok {
...
func init() {
	...
	reserveFundCmd.Flags().StringVar(&reserveFundInDFuelFlag, "fund", "0", "DFuel amount to reserve")
	reserveFundCmd.Flags().StringVar(&reserveCollateralInDFuelFlag, "collateral", "0", "DFuel amount as collateral")
	reserveFundCmd.Flags().StringVar(&feeFlag, "fee", fmt.Sprintf("%dwei", types.MinimumTransactionFeeDFuelWei), "Fee")
