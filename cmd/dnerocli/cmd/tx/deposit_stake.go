...
func doDepositStakeCmd(cmd *cobra.Command, args []string) {
	...
	if purposeFlag == core.StakeForValidator || purposeFlag == core.StakeForSentry {
		dneroStake = stake
	...
	} else if purposeFlag == core.StakeForSentry {
		if strings.HasPrefix(holderFlag, "0x") {
			holderFlag = holderFlag[2:]
		}
		if len(holderFlag) != 458 {
			utils.Error("Holder must be a valid sentry summary")
		}
...
func init() {
	...
	depositStakeCmd.Flags().StringVar(&feeFlag, "fee", fmt.Sprintf("%dwei", types.MinimumTransactionFeeDTokenWeiNewFee), "Fee")
