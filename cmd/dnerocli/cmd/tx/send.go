...
//		dnerocli tx send --chain="privatenet" --path "m/44'/60'/0'/0/0" --to=9F1233798E905E173560071255140b4A8aBd3Ec6 --dnero=10 --dfuel=9 --seq=1 --wallet=trezor
//		dnerocli tx send --chain="privatenet" --path "m/44'/60'/0'/0" --to=9F1233798E905E173560071255140b4A8aBd3Ec6 --dnero=10 --dfuel=9 --seq=1 --wallet=nano
var sendCmd = &cobra.Command{
	Use:     "send",
	Short:   "Send tokens",
	Example: `dnerocli tx send --chain="privatenet" --from=2E833968E5bB786Ae419c4d13189fB081Cc43bab --to=9F1233798E905E173560071255140b4A8aBd3Ec6 --dnero=10 --dfuel=9 --seq=1`,
	Run:     doSendCmd,
}
...
func doSendCmd(cmd *cobra.Command, args []string) {
	...
	dfuel, ok := types.ParseCoinAmount(dfuelAmountFlag)
	if !ok {
		utils.Error("Failed to parse dfuel amount")
	}
	...
	inputs := []types.TxInput{{
		Address: fromAddress,
		Coins: types.Coins{
			DFuelWei: new(big.Int).Add(dfuel, fee),
			DneroWei: dnero,
		},
...
func init() {
	...
	sendCmd.Flags().StringVar(&dfuelAmountFlag, "dfuel", "0", "DFuel amount")
	sendCmd.Flags().StringVar(&feeFlag, "fee", fmt.Sprintf("%dwei", types.MinimumTransactionFeeDFuelWei), "Fee")
