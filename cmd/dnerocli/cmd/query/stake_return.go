...
// Example:
//		dnerocli query stake_returns --height=10
var stakeReturnsCmd = &cobra.Command{
	Use:     "stake_returns",
	Short:   "Get stake returns",
	Example: `dnerocli query stake_returns, dnerocli query stake_returns --height=800`,
...
func doStakeReturnsCmd(cmd *cobra.Command, args []string) {
	...
	var err error
	if height == 0 {
		res, err = client.Call("dnero.GetAllPendingEliteEdgeNodeStakeReturns", rpc.GetAllPendingEliteEdgeNodeStakeReturnsArgs{})
	} else {
		res, err = client.Call("dnero.GetEliteEdgeNodeStakeReturnsByHeight", rpc.GetEliteEdgeNodeStakeReturnsByHeightArgs{Height: common.JSONUint64(height)})
	}
	if err != nil {
...
func init() {
	stakeReturnsCmd.Flags().Uint8Var(&purposeFlag, "purpose", uint8(2), "purpose of the stake return query, validator_node=0, sentry_node=1, elite_edge_node=2")
