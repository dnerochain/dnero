...
func TestValidateInputsBasic(t *testing.T) {
	...
	inputs[0].Coins.DneroWei = big.NewInt(-1)
	res = validateInputsBasic(inputs)
...
func TestValidateOutputsBasic(t *testing.T) {
	...
	tx[0].Coins.DneroWei = big.NewInt(-1)
	res = validateOutputsBasic(tx)
...
// func TestCalculateDneroReward(t *testing.T) {
// 	assert := assert.New(t)

// 	res := calculateDneroReward(big.NewInt(1e17), true)
// 	assert.True(res.DneroWei.Cmp(types.Zero) == 0) // ZERO Dnero inflation
// }
...
// 	// //Error if reward Dnero amount is incorrect
// 	// tx = &types.CoinbaseTx{
// 	// 	Proposer: types.TxInput{
...
func TestReleaseFundTx(t *testing.T) {
	...
	releaseFundTx = &types.ReleaseFundTx{
		Fee: types.NewCoins(100, getMinimumTxFee()), // Dnero cannot be used as transaction fee
		Source: types.TxInput{
			Address:  user1.Address,
			Sequence: 2,
		},
