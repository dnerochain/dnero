...
var dfuelRewardPerBlock = big.NewInt(1).Mul(big.NewInt(48), weiMultiplier) // 48 DFUEL per block, corresponds to about 5% *initial* annual inflation rate. The inflation rate naturally approaches 0 as the chain grows.
...
func grantValidatorReward(ledger core.Ledger, view *st.StoreView, validatorSet *core.ValidatorSet, accountReward *map[string]types.Coins, blockHeight uint64) {
	...
	totalReward := big.NewInt(1).Mul(dfuelRewardPerBlock, big.NewInt(common.CheckpointInterval))
...
func grantStakerRewardRandomized(ledger core.Ledger, view *st.StoreView, validatorSet *core.ValidatorSet, guardianVotes *core.AggregatedVotes,
	...
	samples := make([]*big.Int, dfuelRewardN)
	for i := 0; i < dfuelRewardN; i++ {
	...
	for i := 0; i < len(stakeSourceList); i++ {
		...
		for curr < dfuelRewardN && samples[curr].Cmp(lower) >= 0 && samples[curr].Cmp(upper) < 0 {
			count++
			curr++
		}
		currSum = upper
		...
		if count > 0 {
			...
			rewardAmount := tmp.Div(tmp, big.NewInt(int64(dfuelRewardN)))
