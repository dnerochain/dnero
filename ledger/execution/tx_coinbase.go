...
var dtokenRewardPerBlock = big.NewInt(1).Mul(big.NewInt(48), weiMultiplier)    // 48 DTOKEN per block, corresponds to about 5% *initial* annual inflation rate. The inflation rate naturally approaches 0 as the chain grows.
var eenDtokenRewardPerBlock = big.NewInt(1).Mul(big.NewInt(38), weiMultiplier) // 38 DTOKEN per block, corresponds to about 4% *initial* annual inflation rate. The inflation rate naturally approaches 0 as the chain grows.
var dtokenRewardN = 400                                                        // Reward receiver sampling params
...
// grant block rewards to both the validators and active guardians (they are both dnero stakers)
...
func grantEliteEdgeNodeReward(ledger core.Ledger, view *st.StoreView, guardianVotes *core.AggregatedVotes, eliteEdgeNodeVotes *core.AggregatedEENVotes,
	...
	// the source of the stake divides the block reward proportional to their stake
	totalReward := big.NewInt(1).Mul(eenDtokenRewardPerBlock, big.NewInt(common.CheckpointInterval))
