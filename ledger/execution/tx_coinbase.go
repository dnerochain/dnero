...
var dtokenRewardPerBlock = big.NewInt(1).Mul(big.NewInt(34), weiMultiplier)    // 34 DTOKEN per block, corresponds to about 5% *initial* annual inflation rate. The inflation rate naturally approaches 0 as the chain grows.
var eenTfuelRewardPerBlock = big.NewInt(1).Mul(big.NewInt(14), weiMultiplier) // 14 DTOKEN per block, corresponds to about 4% *initial* annual inflation rate. The inflation rate naturally approaches 0 as the chain grows.
var dtokenRewardN = 100                                                        // Reward receiver sampling params
...
func (exec *CoinbaseTxExecutor) sanityCheck(chainID string, view *st.StoreView, viewSel core.ViewSelector, transaction types.Tx) result.Result {
	...
	sentryPool, eliteEdgeNodePool := RetrievePools(exec.consensus.GetLedger(), exec.chain, exec.db, tx.BlockHeight, sentryVotes, eliteEdgeNodeVotes)
	expectedRewards = CalculateReward(exec.consensus.GetLedger(), view, validatorSet, sentryVotes, sentryPool, eliteEdgeNodeVotes, eliteEdgeNodePool)
...
func RetrievePools(ledger core.Ledger, chain *blockchain.Chain, db database.Database, blockHeight uint64, sentryVotes *core.AggregatedVotes,
	eliteEdgeNodeVotes *core.AggregatedEENVotes) (sentryPool *core.SentryCandidatePool, eliteEdgeNodePool core.EliteEdgeNodePool) {
	sentryPool = nil
	eliteEdgeNodePool = nil

	if blockHeight < common.HeightEnableDneroV1 {
		sentryPool = nil
		eliteEdgeNodePool = nil
	} else if blockHeight < common.HeightEnableDneroV2 {
		if sentryVotes != nil {
			guradianVoteBlock, err := chain.FindBlock(sentryVotes.Block)
			if err != nil {
				logger.Panic(err)
			}
			storeView := st.NewStoreView(guradianVoteBlock.Height, guradianVoteBlock.StateHash, db)
			sentryPool = storeView.GetSentryCandidatePool()
		}
	} else { // blockHeight >= common.HeightEnableDneroV2
		// won't reward the elite edge nodes without the sentry votes, since we need to sentry votes to confirm that
		...
		if sentryVotes != nil {
			...
			if eliteEdgeNodeVotes != nil {
				if eliteEdgeNodeVotes.Block == sentryVotes.Block {
					eliteEdgeNodePool = st.NewEliteEdgeNodePool(storeView, true)
				} else {
					logger.Warnf("Elite edge nodes vote for block %v, while sentrys vote for block %v, skip rewarding the elite edge nodes",
						eliteEdgeNodeVotes.Block.Hex(), sentryVotes.Block.Hex())
				}
			} else {
				logger.Warnf("Elite edge nodes have no vote for block %v", sentryVotes.Block.Hex())
			}
		}
	}

	return sentryPool, eliteEdgeNodePool
}
...
func CalculateReward(ledger core.Ledger, view *st.StoreView, validatorSet *core.ValidatorSet,
	sentryVotes *core.AggregatedVotes, sentryPool *core.SentryCandidatePool,
	...
	if blockHeight < common.HeightEnableValidatorReward {
		...
		grantValidatorAndSentryReward(ledger, view, validatorSet, sentryVotes, sentryPool, &accountReward, blockHeight)
	} else { // blockHeight >= common.HeightEnableDneroV2
		grantValidatorAndSentryReward(ledger, view, validatorSet, sentryVotes, sentryPool, &accountReward, blockHeight)
		grantEliteEdgeNodeReward(ledger, view, sentryVotes, eliteEdgeNodeVotes, eliteEdgeNodePool, &accountReward, blockHeight)
	}
...
func grantValidatorReward(ledger core.Ledger, view *st.StoreView, validatorSet *core.ValidatorSet, accountReward *map[string]types.Coins, blockHeight uint64) {
	...
	totalReward := big.NewInt(1).Mul(dtokenRewardPerBlock, big.NewInt(common.CheckpointInterval))
	...
	for stakeSourceAddr, stakeAmountSum := range stakeSourceMap {
		...
		reward := types.Coins{
			DneroWei: big.NewInt(0),
			DTokenWei: rewardAmount,
		}.NoNil()
...
// grant block rewards to both the validators and active sentrys (they are both dnero stakers)
func grantValidatorAndSentryReward(ledger core.Ledger, view *st.StoreView, validatorSet *core.ValidatorSet, sentryVotes *core.AggregatedVotes,
	...
	if blockHeight < common.HeightSampleStakingReward {
		...
		issueRandomizedReward(ledger, sentryVotes, view, effectiveStakes,
			totalStake, accountReward, totalReward, srdsr, "Block")
	}
}

// grant uptime mining rewards to active elite edge nodes (they are the dtoken stakers)
func grantEliteEdgeNodeReward(ledger core.Ledger, view *st.StoreView, sentryVotes *core.AggregatedVotes, eliteEdgeNodeVotes *core.AggregatedEENVotes,
	...
	if sentryVotes == nil {
		// Should never reach here
		panic("sentryVotes == nil")
	}

	logger.Debugf("grantEliteEdgeNodeReward: sentryVotes = %v, eliteEdgeNodeVotes = %v", sentryVotes, eliteEdgeNodeVotes)
...
func issueRandomizedReward(ledger core.Ledger, sentryVotes *core.AggregatedVotes, view *st.StoreView, effectiveStakes [][]*core.Stake,
	...
	samples := make([]*big.Int, dtokenRewardN)
	for i := 0; i < dtokenRewardN; i++ {
	...
	if srdsr != nil {
		...
		for _, stakes := range effectiveStakes {
			for _, stake := range stakes {
				...
				for curr < dtokenRewardN && samples[curr].Cmp(lower) >= 0 && samples[curr].Cmp(upper) < 0 {
					count++
					curr++
				}
				currSum = upper
				...
				if count > 0 {
					...
					rewardAmount := tmp.Div(tmp, big.NewInt(int64(dtokenRewardN)))
