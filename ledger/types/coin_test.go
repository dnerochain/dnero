...
func TestInvalidCoin(t *testing.T) {
	...
	ret1.DneroWei = big.NewInt(456)
	assert.True(coinsA.DneroWei.Cmp(big.NewInt(123)) == 0)
	assert.True(ret1.DneroWei.Cmp(big.NewInt(456)) == 0)

	ret2 := coinsA.Minus(coinsEmpty)
	assert.True(coinsA.IsEqual(ret2), "Sum is correct")
	// Result should be a copy
	ret2.DneroWei = big.NewInt(456)
	assert.True(coinsA.DneroWei.Cmp(big.NewInt(123)) == 0)
	assert.True(ret2.DneroWei.Cmp(big.NewInt(456)) == 0)
}
