// Copyright 2020 The go-ethereum Authors, Adapted to Dnero
...
func TranslateEthTx(ethTxStr string) (*SmartContractTx, error) {
	...
	coins := Coins{
		...
		DTokenWei: ethTx.value(),
	}

	from := TxInput{
		Address:   fromAddr,
		Coins:     coins,
		Sequence:  ethTx.nonce() + 1, // off-by-one, ETH tx nonce starts from 0, while Dnero tx sequence starts from 1
		Signature: sig,
	}
