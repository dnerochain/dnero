...
// GetSentryCandidatePool gets the sentry candidate pool.
func (sv *StoreView) GetSentryCandidatePool() *core.SentryCandidatePool {
	...
	err := types.FromBytes(data, scp)
	if err != nil {
		log.Panicf("Error reading sentry candidate pool %X, error: %v",
			data, err.Error())
	}
	return scp
}
...
func (sv *StoreView) UpdateSentryCandidatePool(scp *core.SentryCandidatePool) {
	scpBytes, err := types.ToBytes(scp)
	if err != nil {
		log.Panicf("Error writing sentry candidate pool %v, error: %v",
			scp, err.Error())
	}
	sv.Set(SentryCandidatePoolKey(), scpBytes)
}
...
func (sv *StoreView) Suicide(addr common.Address) bool {
	...
	sv.addBalanceChange(&types.BalanceChange{
		...
		TokenType:  1,
		IsNegative: true,
		Delta:      new(big.Int).Set(account.Balance.DTokenWei),
	})
